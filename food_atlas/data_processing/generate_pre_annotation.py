...
TO_PREDICT_FILEPATH = "../../outputs/data_generation/to_predict.tsv"
...
def main():
    ...
    df_val.drop('source', axis=1, inplace=True)
    df_test.drop('source', axis=1, inplace=True)
    df_to_predict = pd.concat([df_ph_pairs, df_train, df_val, df_test])
    df_to_predict.drop_duplicates(keep=False, inplace=True)
    df_to_predict["source"] = "prediction"
    print(f"Saving to-predict data to {args.to_predict_filepath}")
