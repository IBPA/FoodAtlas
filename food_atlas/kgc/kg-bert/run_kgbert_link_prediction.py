...
class FA_KGProcessor(DataProcessor):
    """Processor for knowledge graph data set."""
    def __init__(self):
        self.labels = set()
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        val_test_dir = str(Path(data_dir).parents[0])
        lines = self._read_tsv(os.path.join(val_test_dir, "val.txt"))
        return self._create_examples(lines, "dev", data_dir)

    def get_test_examples(self, data_dir):
        """See base class."""
        val_test_dir = str(Path(data_dir).parents[0])
        lines = self._read_tsv(os.path.join(val_test_dir, "test_corrupted.txt"))
    ...
    def get_relations(self, data_dir):
        ...
        return df_relations['foodatlas_id'].tolist()
    ...
    def get_entities(self, data_dir):
        ...
        return df_entities['foodatlas_id'].tolist()
    ...
    def get_dev_triples(self, data_dir):
        """Gets validation triples."""
        val_test_dir = str(Path(data_dir).parents[0])
        return self._read_tsv(os.path.join(val_test_dir, "val.txt"))

    def get_test_triples(self, data_dir):
        """Gets test triples."""
        val_test_dir = str(Path(data_dir).parents[0])
        return self._read_tsv(os.path.join(val_test_dir, "test.txt"))
    ...
    def _create_examples(self, lines, set_type, data_dir):
        ...
        rel2text = dict(zip(
            ...
            df_relations['translation'].tolist(),
        ))

        lines_str_set = set([
            '\t'.join(line) if len(line) == 3 else '\t'.join(line[:3]) for line in lines
        ])
        examples = []
        for (i, line) in enumerate(lines):
            head_ent_text = ent2text[line[0]]
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # sequence or the second sequence or the third sequence. The embedding vectors
        # for `type=0` and
...
def main():
    ...
    parser.add_argument(
        "--num_eval",
        type=int,
        help="How many triples to use when evaluating.")
    parser.add_argument(
...
def do_eval(args, processor, label_list, num_labels, device, entity_list):
    ...
    if args.num_eval:
        print(f'Running link prediction on smaller dataset {args.num_eval} '
              f'instead of {len(dev_triples)}')
        dev_triples = dev_triples[:args.num_eval]
    else:
        print(f'Running link prediction on full dataset {len(dev_triples)}')

    for dev_triple in tqdm(dev_triples, desc='Dev triple: '):
        head = dev_triple[0]
        relation = dev_triple[1]
        tail = dev_triple[2]

        head_corrupt_list = [dev_triple]
        ...
        # for input_ids, input_mask, segment_ids, label_ids in \
        #         tqdm(eval_dataloader, desc="Testing"):
        ...
        with open(os.path.join(args.output_dir, 'ranks.txt'), 'a') as _f:
            _f.write(str(rank1) + '\t' + str(rank2) + '\n')
    ...
    lp_metrics = {}
    for i in [0, 2, 9]:
        lp_metrics[f'Hits left {i+1}'] = [np.mean(hits_left[i])]
        lp_metrics[f'Hits right {i+1}'] = [np.mean(hits_right[i])]
        lp_metrics[f'Hits {i+1}'] = [np.mean(hits[i])]
    lp_metrics['Mean rank left'] = [np.mean(ranks_left)]
    lp_metrics['Mean rank right'] = [np.mean(ranks_right)]
    lp_metrics['Mean rank'] = [np.mean(ranks)]
    lp_metrics['Mean reciprocal rank left'] = [np.mean(1./np.array(ranks_left))]
    lp_metrics['Mean reciprocal rank right'] = [np.mean(1./np.array(ranks_right))]
    lp_metrics['Mean reciprocal rank'] = [np.mean(1./np.array(ranks))]

    df_metrics = pd.DataFrame(lp_metrics)
    df_metrics.to_csv(
        os.path.join(args.output_dir, 'eval_link_prediction_metrics.tsv'),
        sep='\t', index=False
    )

    # configs
    with open(os.path.join(args.output_dir, 'args.json'), 'w') as _f:
        json.dump(vars(args), _f)
...
def do_predict(
    ...
    report = generate_report(all_label_ids, preds)
    df_report = pd.Series(report).to_frame().transpose()
    df_report.to_csv(os.path.join(args.output_dir, 'test_results.txt'), sep='\t', index=False)
